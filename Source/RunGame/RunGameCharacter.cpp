...
void ARunGameCharacter::OnDeathMontageBlendingOut(UAnimMontage* Montage, bool bInterrupted)
{
	...
	//彻底清空并禁用移动组件
	GetCharacterMovement()->StopMovementImmediately(); // 刹停当前的所有惯性和速度
	GetCharacterMovement()->DisableMovement();         // 禁用物理计算

	//冻结动画网格体
	if (USkeletalMeshComponent* SkelMesh = GetMesh())
	{
		SkelMesh->bPauseAnims = true;          // 暂停动画播放（定格在当前帧）
		SkelMesh->bNoSkeletonUpdate = true;    // 彻底关闭骨骼更新（极大节省 CPU 性能）
	}
	StartDissolve();
}
